...
import servicesData from "./AdditionalServices.json";
...
const AdditionalServices = () => {
  ...
  useEffect(() => {
    setServices(servicesData.sections);
  }, []);
  ...
  const renderContent = (content) => {
    return content.map((item, index) => {
      switch (item.type) {
        ...
        case "table":
          return (
            <div key={index} className={styles["wrap-table"]}>
              ...
              <Table
                ...
                dataSource={item.rows.map((row, idx) => {
                  const rowData = {};
                  item.columns.forEach((col, i) => {
                    rowData[col] = row[i];
                  });
                  return { ...rowData, key: idx };
                })}
                pagination={false}
              />
            </div>
        ...
        <Collapse accordion className={styles.accordion}>
          {services.map((section, index) => (
            <Panel
              ...
              key={index}
              className={styles["accordion-row"]}
            >
              ...
              <Paragraph>
                ...
                тел.:{" "}
                <a
                  href={`tel:+${section.contacts.phone.replace(/[^0-9]/g, "")}`}
                >
                  <b>{section.contacts.phone}</b>
                </a>{" "}
                доб. {section.contacts.extensions.join(", ")}; e-mail:{" "}
                <a href={`mailto:${section.contacts.email}`}>
                  <b>{section.contacts.email}</b>
                </a>
              </Paragraph>
...
// import React, { useEffect, useState } from "react";
// import { Typography, Card, Table, Collapse } from "antd";
// import servicesData from "./AdditionalServices.json";
...
//               {section.tables && section.tables.map((table, idx) => (
//                 <div key={idx} style={{ marginTop: "20px" }}>
//                   {table.title && <Title level={4}>{table.title}</Title>}
//                   {renderTable(table)}
//                 </div>
//               ))}
//             </Panel>
