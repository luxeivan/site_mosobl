...
export default function Filial() {
  ...
  useEffect(() => {
    fetch(
      `${addressServer}/api/filialies/${params.id}?populate[0]=proizvodstvennye_otdeleniyas&populate[1]=kontakties&populate[2]=proizvodstvennye_otdeleniyas.kontakties&populate[3]=work_schedule&populate[4]=proizvodstvennye_otdeleniyas.work_schedule`
    )
      .then((response) => {
        ...
        <section className="inner-post">
          <div className="container">
            <div className="inner-post__up">
              ...
              <h1 className="inner-post__title inner-post__margin">
                {filial.attributes && filial.attributes.name}
              </h1>
              <span className="inner-post__date inner-post__margin">
                {filial.attributes && filial.attributes.address}{" "}
              </span>
              <div
                style={{
                  display: "flex",
              ...
              >
                {filial.attributes &&
                  filial.attributes.work_schedule.map((item, index) => (
                    <p key={index}>
                      <span className="inner-post__date ">{item.days} - </span>
                      <span
                        style={{ fontWeight: 700, marginLeft: 0 }}
                        className="inner-post__date inner-post__margin"
                      >
                        {item.times}
                      </span>
                    </p>
            ...
            <div className="inner-post__middle">
                        ...
                        <div className="positions-post__wrapper">
                          ...
                          <div className="positions-post__down">
                            <div className="positions-post__row-name">
                              <h4 className="positions-post__name">
                                {item.attributes.post}
                              </h4>
                            </div>
                            <div className="positions-post__wrap-text">
                              <span>
                                тел.:
                                <a
                                  className="positions-post__tel"
                                  href={`tel:${item.attributes.tel}`}
                                >
                                  {item.attributes.tel}
                                </a>
                              </span>
              ...
              {filial.attributes &&
                filial.attributes.proizvodstvennye_otdeleniyas.data.map(
                  (item) => (
                      ...
                      <div className="accordion-row__drop-down">
                        <div className="accordion-row__wrapper">
                          <div className="text-area">
                            ...
                            <div>
                              {item.attributes.work_schedule?.map(
                                (item, index) => (
                                  <p style={{ marginBottom: 0 }} key={index}>
                                    {item.days} - <em>{item.times}</em>
                                  </p>
                                )
                              )}
                            </div>
