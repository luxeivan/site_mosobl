...
export default function SecretDisconnect() {
  ...
  // const highlightStyle = { backgroundColor: "#FF6347" }; 
  const highlightStyle = { backgroundColor: "#FFA07A", animation: "blink 1s infinite" }; // Стиль для выделения новых записей
            ...
            >
              ...
              {disconnects.map((disconnect, index) => (
                <div key={index} style={{ display: "flex", alignItems: "center" }}>
                  <p style={isToday(disconnect.addedAt) ? highlightStyle : {}}>
                    {disconnect.text}
                  </p>
                  {isToday(disconnect.addedAt) && (
                    <span style={{ marginLeft: "10px", color: "gray" }}>
                      {DateTime.fromMillis(disconnect.addedAt).toFormat("HH:mm")}
                    </span>
                  )}
                </div>
              ))}
...
//                 <p
//                   key={index}
//                   style={isToday(disconnect.addedAt) ? highlightStyle : {}}
//                 >
//                   {disconnect.text}
//                 </p>
//               ))}
